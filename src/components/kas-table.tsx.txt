...
export function KasTable({ rows }: { rows: Row[] }) {
  ...
  async function handleExport() {
    ...
    const data = filtered.map((r) => ({
      Tanggal: formatDate(r.transaction_date),
      Tipe: r.type === "masuk" ? "Kas Masuk" : "Kas Keluar",
      Kategori: r.category ?? "-",
    ...
    data.push({
      ...
      Keterangan: "TOTAL SALDO (sesuai filter)",
      Jumlah: saldo,
    } as any);
    ...
    const filename = dateFrom || dateTo
      ? `kas_${dateFrom || "awal"}_sd_${dateTo || "akhir"}.xlsx`
      : "kas_semua.xlsx";
    ...
    <div className="space-y-4">
      ...
      <div className="card">
        <div className="flex flex-wrap gap-2.5 mb-5 items-center justify-between">
          ...
          <button onClick={handleExport} className="text-xs font-semibold bg-orange-soft text-orange-deep rounded-pill px-4 py-2">
            Export Excel
          </button>
