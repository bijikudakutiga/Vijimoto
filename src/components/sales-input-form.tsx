...
export function SalesInputForm({
  ...
  async function handleCreateInvoice() {
    ...
    await supabase.from("invoices").insert({ sale_id: createdSale.id });
    router.push("/penjualan/status");
  }

  async function handleCreateSuratJalan() {
    if (!createdSale) return;
    await supabase.from("surat_jalan").insert({ sale_id: createdSale.id });
    router.push("/penjualan/status");
  }
      ...
      <div className="card max-w-lg">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button
            ...
            className="bg-teal text-white text-sm font-semibold rounded-pill px-4 py-2"
          >
            Buat Surat Jalan
          </button>
          <button
      ...
      <div className="card">
        ...
        <div className="space-y-2">
          ...
          {items.map((it) => (
            <div
              key={it.key}
              className="border border-line rounded-2xl p-3 grid grid-cols-[1.6fr_0.8fr_0.6fr_1fr_1fr_auto] gap-2 items-end"
            >
              <div>
